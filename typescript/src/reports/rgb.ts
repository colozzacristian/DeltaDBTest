...
export type Color = { type: "rgb"; r: number; g: number; b: number } | {
  type: "rainbow";
};

export function writeColorToFormat(
  color: Color,
  kind: KeyboardKind,
  buf: Uint8Array,
) {
...
export function effectToU8(
  ...
  if (kind === KeyboardKind.AK820) {
    switch (effect) {
      case Effect.Static:
        return 5;
      case Effect.Corrugated:
        return 1;
      case Effect.Cloud:
        return 2;
      case Effect.Serpentine:
        return 3;
      case Effect.Spectrum:
        return 4;
      case Effect.Breath:
        return 5;
      case Effect.Reaction:
        return 7;
      case Effect.Ripples:
        return 8;
      case Effect.Traverse:
        return 9;
      case Effect.Stars:
        return 10;
      case Effect.Flowers:
    ...
    switch (effect) {
      case Effect.Static:
        return 1;
      case Effect.Glittering:
        return 4;
      case Effect.Falling:
        return 5;
      case Effect.Colourful:
        return 6;
      case Effect.Breath:
        return 7;
      case Effect.Spectrum:
...
export function writeEffectToFormat(
  effect: Effect,
  kind: KeyboardKind,
  buf: Uint8Array,
) {
...
export function writeBrightnessToFormat(
  brightness: Brightness,
  kind: KeyboardKind,
  buf: Uint8Array,
) {
  if (
    kind === KeyboardKind.AK820 || kind === KeyboardKind.AK35I ||
    kind === KeyboardKind.F75_MAX
  ) {
...
export function writeSpeedToFormat(
  speed: Speed,
  kind: KeyboardKind,
  buf: Uint8Array,
) {
...
export function writeDirectionToFormat(
  direction: Direction,
  kind: KeyboardKind,
  buf: Uint8Array,
) {
...
export async function writeRgbInto(
  rgb: Rgb,
  keyboard: Keyboard,
): Promise<void> {
